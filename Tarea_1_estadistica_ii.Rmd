...
Inicialmente el paquete _ks_ de R es un paquete orientado a suavizar datos univariados o multivariados mediante kernels. Dentro de este paquete se encuentra la función _Kernel density estimate (kde)_ la cual estima la función de densidad de probabilidad para datos de 1 a 6 dimensiones de forma no paramétrica. 

```{r}
KDE <- kde(BD$Salario)
plot(KDE, main = "Función estimada de densidad de probabilidad", ylab = "", xlab = "")
```

**2.Del paquete boot de R, describa brevemente la función boot.ci**

Este paquete contiene funciones y bases de datos para bootstrapping del libro _`Bootstrap Methods and Their Application_. La función boot.ci genera 5 tipos diferentes de intervalos de confianza no paramétricos de dos colas iguales: Itervalos de Apriximación normal, intevalos básicos bootstrap, intervalos bootstrap studentnizados, intervalos percentil bootstrap e intervalos ajustados percentil bootstrap, de lo cuales se puede hacer una selección según preferencia.
...
**3.Realice un Bootstarp de la media de los U. Salarios sin filtrar por sexo, (para esto deben crear una función que contenga índices) utilizando la función boot.**

__a.Estime la media Bootstrap (ver la variable t) y compárela con la media de los datos originales (ver la variable t0).__
...
__b.Realice un histograma con los resultados del Bootstrap.__
Resulta evidente que, aunque existe una diferencia en los resultados, esta no es de gran magnitud, mostrándose así que la media obtenida mediante el método boostrap es una aproximación muy certera para el caso en cuestión.
